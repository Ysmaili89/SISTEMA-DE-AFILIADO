...
def admin_advertisements():
    advertisements = Advertisement.query.order_by(Advertisement.id.desc()).all()
    form = AdvertisementForm()  # Instancia el formulario aquí
    return render_template('admin/admin_advertisements.html', advertisements=advertisements, form=form)
...
# ... (las otras rutas de edición y eliminación de anuncios continuarían aquí)
...
def admin_edit_advertisement(ad_id):
    ...
    if request.method == 'GET':
        # Assign the product object to the form field, not the ID
        form.product.data = advertisement.product

    if form.validate_on_submit():
        # Correctly get the product ID from the QuerySelectField
...
# NEW ROUTE TO TOGGLE ADVERTISEMENT ACTIVE STATUS
@bp.route('/advertisements/toggle_active/<int:ad_id>', methods=['POST'])
@admin_required
def admin_toggle_active_advertisement(ad_id):
    advertisement = Advertisement.query.get_or_404(ad_id)
    try:
        advertisement.is_active = not advertisement.is_active
        db.session.commit()
        status = "active" if advertisement.is_active else "inactive"
        flash(f'Advertisement "{advertisement.title}" marked as {status} successfully.', 'info')
    except Exception as e:
        db.session.rollback()
...
def admin_add_affiliate():
    ...
    if form.validate_on_submit():
        new_affiliate = Afiliado(
            nombre=form.nombre.data,
            email=form.email.data, # Corrected from link_afiliado
            enlace_referido=form.enlace_referido.data, # Corrected from comision_porcentaje
...
def admin_add_affiliate_statistic():
    ...
    # This section needs to be checked. QuerySelectField auto-populates.
    # The form.afiliado field is a QuerySelectField and doesn't have a `choices` attribute like a normal SelectField.
    # It will populate itself, but the view code should use the correct field name.
    ...
    if form.validate_on_submit():
        # Correctly get the affiliate from the QuerySelectField
        selected_affiliate = form.afiliado.data
        if selected_affiliate:
            new_stat = EstadisticaAfiliado(
                afiliado_id=selected_affiliate.id,
                fecha=form.start_date.data, # Corrected field name
...
def admin_edit_affiliate_statistic(stat_id):
    ...
    if request.method == 'GET':
        # Assign the affiliate object to the form field, not the ID
        form.afiliado.data = stat.afiliado
    ...
    if form.validate_on_submit():
        ...
        if selected_affiliate:
            ...
            stat.afiliado_id = selected_affiliate.id
            stat.fecha = form.start_date.data # Corrected field name
            try:
                db.session.commit()
